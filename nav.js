    ...
    <div class="modal-overlay" id="modal-2fa" style="z-index:1000">
      <div class="modal" style="max-width:420px">
        ...
        <div id="2fa-reenviar-wrap" style="display:none;text-align:center;margin-top:0.8rem">
          <a style="color:var(--muted);font-size:0.82rem;cursor:pointer" onclick="window._reenviar2FA()">
...
window._reenviar2FA = async () => {
  ...
  if (ok) showToast('Código reenviado!');
...
export function initAuth() {
  ...
  onAuthStateChanged(auth, async (user) => {
    ...
    if (user) {
      ...
      // ── 2FA — D7, D8, D9 ──────────────────────────────
      const precisa2FA = isAdmin || roles.includes('breeder') || roles.includes('vendedor') || true;
      // 'true' garante que todos passam pelo 2FA (usuários comuns usam TOTP)
